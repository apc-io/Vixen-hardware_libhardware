...
#ifdef FIX_DISPLAY_EXTERNAL
static int release_framebuffer_if_display_changed(struct private_module_t* module)
{
	if (module->framebuffer != NULL) {
		struct fb_var_screeninfo info;
		if (ioctl(module->framebuffer->fd, FBIOGET_VSCREENINFO, &info) == -1)
			return -errno;
		if (module->info.yres != info.yres)
		{
			// release framebuffer due to display resolution was changed.
			close(module->framebuffer->fd);
...
#ifdef FIX_DISPLAY_EXTERNAL
	release_framebuffer_if_display_changed(m);
#endif
...
static int alloc_device_alloc(alloc_device_t *dev, int w, int h, int format, int usage, buffer_handle_t *pHandle, int *pStride)
{
	...
	if (usage & GRALLOC_USAGE_HW_FB)
	{
		extern unsigned int gralloc_bits;
		if( gralloc_bits == 32 )
