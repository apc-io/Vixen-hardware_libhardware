...
#define ENV_NAME_GRALLOC "wmt.gralloc.param"
#define MAX_BUFFER_LENGTH 24
char *envbuf = NULL;
unsigned int gralloc_bits=16;

extern "C" {
	int wmt_getsyspara(const char *varname, char *varval, int * varlen);
}

static int get_uboot_gralloc_param(const char *name)
{
...
int init_frame_buffer_locked(struct private_module_t *module)
{
	...
	if ( get_uboot_gralloc_param(ENV_NAME_GRALLOC) == 0 ) {
		sscanf(envbuf,"%x",&gralloc_bits);
	}
	ALOGI("wmt.gralloc.param=[%d]\n",  gralloc_bits);
	if( gralloc_bits != 32 )
		gralloc_bits = 16;
	...
	if (int(info.width) <= 0 || int(info.height) <= 0)
	{
		...
		dpi = (info.xres == 1920) ? 240 : 160;

		// the driver doesn't return that information
		// default to 160 dpi
		info.width  = ((info.xres * 25.4f)/dpi + 0.5f);
		info.height = ((info.yres * 25.4f)/dpi + 0.5f);
	}
